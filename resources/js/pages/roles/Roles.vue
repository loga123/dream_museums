        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <div>
                <h3 class="card-title">{{$t('roles')}} &nbsp</h3>
                <i v-if="$store.state.auth.user && $can($store.state.auth.user,'role-create')" class="fa fa-plus-square" @click="newModal"></i>
              </div>
            ...
            <div class="card-body table-responsive p-0">
              <b-table sticky-header id="Gradovi" no-local-sorting responsive hover :items="roles.data" :fields="fieldsRoles"  :sort-by.sync="sortBy"  :sort-desc.sync="sortDesc"  @sort-changed="getResults(1)">
                ...
                <template v-slot:cell(name)="data">
                  <router-link :to="{ name: 'showRole', params: {id: data.item.id } }">
                    {{ data.value}}
                  </router-link>
                </template>

                <template v-slot:cell(action)="data" v-if="$store.state.auth.user">
                  <a  v-if="$can($store.state.auth.user,'role-edit')" href="#" @click="editModal(data.item)">
                    <i class="fa fa-edit blue"></i>
                  </a>

                  <a  v-if="$can($store.state.auth.user,'role-delete')" href="#" @click="deleteRole(data.item)">
            ...
            <div class="card-footer">
              <pagination :limit=2 :data="roles" @pagination-change-page="getResults" >
                <span slot="prev-nav">&lt; {{$t('previous')}}</span>
                <span slot="next-nav">{{$t('next')}} &gt;</span>
              </pagination>
              <p>{{$t('showing')}} {{this.roles.from}} - {{this.roles.to}} {{$t('to')}} {{this.roles.total}} {{$t('results')}} </p>
            </div>
          </div>
          <!-- /.card -->
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getResults(page = 1) {
        ...
        let route="/api/role?page="+page+"&sort="+sort+"&desc="+sortDesc+"&perPage="+perPage;
        if(query){
          route+="&q="+query;
        }
      ...
      editModal(role){
        ...
        this.form.permissions=arrayList;
      },
      newModal(){
        this.deleteClassModal();
        this.editmode = false;
      ...
      deleteRole(role){
        swal.fire({
          title:  this.$t('delete_modal_title')+ ' "'+role.name+'" ?',
          ...
          if (result.value) {
            this.form.delete('/api/role/'+role.id).then((data)=>{
              if(data.data.success){
                swal.fire(
                  data.data.data,
                  data.data.message,
